...
type (
	...
	Field struct {
		...
		ModelsName string // auto accounts for ID for rel
		ModelsType Type
		...
		NullConstructorFunc string

		ColumnConstant string
		Index          int
	...
	for i, path := range pkg.CompiledGoFiles {
		...
		for _, cg := range file.Comments {
			for _, dir := range pkgutil.ParseDirectives(cg) {
				if dir.Module != "boil" || dir.Directive != "models" {
					continue
				}
...
func findEntities(pkg *packages.Package, mdir ModelsDirective) ([]Entity, error) {
	...
	for _, name := range scope.Names() {
		...
		dirs := pkgutil.FindDirectives(pkg, getterObj, "boil")
		if len(dirs) == 0 {
			continue
		}
		...
		modelObj := mdir.Pkg.Types.Scope().Lookup(e.ModelsName)
		if modelObj == nil {
			return nil, objErr(pkg, getterObj, fmt.Sprintf("found no model type named %q", e.ModelsName))
		}

		model, ok := pkgutil.ElemType(modelObj.Type()).(*types.Struct)
		if !ok {
			return nil, objErr(pkg, getterObj, fmt.Sprintf("expected models.%s to be struct", e.ModelsName))
		}
...
func findFields(pkg *packages.Package, mdir ModelsDirective, getterObj, modelObj types.Object, getter, setter, model, relations *types.Struct) ([]Field, error) {
	...
	for i := 0; i < getter.NumFields(); i++ {
		...
		if tag["boil"] == "-" || (tag["wrap"] == "-" && tag["unwrap"] == "-") {
			continue
		}
		...
		if !(f.SetterType.IsArray && tag["rel"] != "") {
			...
			if name := tag["boil"]; name != "" {
				f.ModelsName = name
			}
			f.ColumnConstant = modelObj.Name() + "Columns." + f.ModelsName
			...
			switch {
			case strings.HasPrefix(modelsTypeName, "null."):
				if modelsTypeName == "null.Time" {
					f.ModelsType = Type{
						Type:       modelsTypeName,
						Elem:       "time.Time",
						IsNullable: true,
					...
					base := strings.ToLower(strings.TrimPrefix(modelsTypeName, "null."))
					f.ModelsType = Type{
						Type:       modelsTypeName,
						Elem:       base,
						IsNullable: true,
				...
				pkg, typ, _ := strings.Cut(modelsTypeName, ".")
				f.NullConstructorFunc = pkg + ".New" + typ
			case strings.HasPrefix(modelsTypeName, "types.") && strings.HasSuffix(modelsTypeName, "Array"):
				base := strings.ToLower(strings.TrimSuffix(strings.TrimPrefix(modelsTypeName, "types."), "Array"))
				f.ModelsType = Type{
					Type:    modelsTypeName,
					Elem:    base,
					IsArray: true,
...
func wrapErr(err error) error {
	...
	return fmt.Errorf("boil: %w", err)
}

func objErr(pkg *packages.Package, obj types.Object, s string) error {
	return pkgutil.PosError(pkg, obj.Pos(), fmt.Errorf("boil: %s: %s", obj.Name(), s))
}
